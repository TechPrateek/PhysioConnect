...
export default async function PatientProfilePage() {
  ...
  const profileRes = await getPatientProfileAction();
  if (!profileRes.success || !profileRes.data) {
    return (
      <div className="container mx-auto p-8 text-center text-sm text-destructive">
        {profileRes.error || "Failed to load patient profile"}
      </div>
    );
  }
      ...
      <main className="container mx-auto max-w-3xl px-4 py-8 space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage your personal contact details, emergency info, and medical history notes for visiting physiotherapists in Etawah.
          </p>
        </div>

        <PatientProfileEditor initialData={profileRes.data} />
      </main>
    </div>
  );
}
